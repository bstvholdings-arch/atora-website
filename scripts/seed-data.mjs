/**
 * Shared demo seed data for ATORA.
 *
 * This module is intentionally plain ESM JavaScript (no TS, no pg import) so it
 * can be consumed by:
 *   - `scripts/seed.mjs`      (real PostgreSQL via pg-helper)
 *   - `src/lib/db.ts`         (in-memory pg-mem fallback for local preview)
 *
 * It only depends on a `db` object exposing the better-sqlite3-style API:
 *   db.prepare(sql).run(...params) / .get(...) / .all(...)
 *   db.exec(multiStatementSql)
 *
 * Admin-user creation is intentionally excluded (handled in seed.mjs) so the
 * pg-mem preview path does not need bcrypt.
 */
export async function seedDatabase(db) {
  console.log('[seed-data] Clearing existing content...');
  await db.exec(`
  ...
  console.log('[seed-data] Site settings...');
  const settings = [
  ...
  console.log('[seed-data] Categories...');
  ...
  for (let i = 0; i < categoryGroups.length; i++) {
    ...
    // NOTE: must await — .run() returns a Promise, so reading lastInsertRowid
    // synchronously would yield undefined and leave every child parentless.
    const info = await stmtParentCat.run(g.slug, g.name, i);
    const pid = info.lastInsertRowid;
    for (let j = 0; j < g.children.length; j++) {
      const child = g.children[j];
      await stmtChildCat.run(`${g.slug}-${child.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`, child, pid, j);
    }
  }
  const catMap = {};
  for (const r of await db.prepare('SELECT id, slug FROM categories').all()) catMap[r.slug] = r.id;

  console.log('[seed-data] Locations...');
  const locs = [
    {
  ...
  console.log('[seed-data] Technical Partners (samples)...');
  const partners = [
    {
  ...
  console.log('[seed-data] Homepage content sections...');
  const sections = [
    {
      key: 'product_videos',
  ...
  console.log('[seed-data] Product media (sample primary images)...');
  const mediaSeed = [
