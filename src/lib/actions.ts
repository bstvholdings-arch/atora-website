 ...
 * BRANDS
 * ============================================================ */
function uniqueBrandSlug(slug: string): string {
  let s = slug;
  let i = 2;
  while (db.prepare('SELECT id FROM brands WHERE slug = ?').get(s)) {
    s = `${slug}-${i}`;
    i++;
  }
  return s;
...
export async function createBrandAction(formData: FormData): Promise<void> {
  ...
  if (!name_en) return;
  const slug = uniqueBrandSlug(slugify(name_en));

  db.prepare(
    `INSERT INTO brands (slug, name_en, name_bm, name_zh, logo, description_en, description_bm, description_zh, display_order, featured, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    slug,
    name_en,
    formData.get('name_bm')?.toString() || null,
    formData.get('name_zh')?.toString() || null,
    formData.get('logo')?.toString() || null,
    formData.get('description_en')?.toString() || null,
    formData.get('description_bm')?.toString() || null,
    formData.get('description_zh')?.toString() || null,
    Number(formData.get('display_order') || 0),
    formData.get('featured') ? 1 : 0,
    formData.get('status') === 'off' ? 0 : 1
  );
 ...
 * CATEGORIES
 * ============================================================ */
function uniqueCategorySlug(slug: string): string {
  let s = slug; let i = 2;
  while (db.prepare('SELECT id FROM categories WHERE slug = ?').get(s)) { s = `${slug}-${i}`; i++; }
  return s;
}
...
export async function createCategoryAction(formData: FormData): Promise<void> {
  ...
  const name_en = formData.get('name_en')?.toString().trim() ?? '';
  if (!name_en) return;
  const slug = uniqueCategorySlug(slugify(name_en));
  db.prepare(
    `INSERT INTO categories (slug, name_en, name_bm, name_zh, parent_id, icon, display_order, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    slug,
    name_en,
    formData.get('name_bm')?.toString() || null,
    formData.get('name_zh')?.toString() || null,
    Number(formData.get('parent_id') || 0) || null,
    formData.get('icon')?.toString() || null,
    Number(formData.get('display_order') || 0),
    formData.get('status') === 'off' ? 0 : 1
  );
...
export async function updateCategoryAction(id: number, formData: FormData): Promise<void> {
  ...
  if (!admin) redirect('/admin/login');
  db.prepare(
    `UPDATE categories SET name_en = ?, name_bm = ?, name_zh = ?, parent_id = ?, display_order = ?, status = ? WHERE id = ?`
  ).run(
    formData.get('name_en')?.toString() || '',
    formData.get('name_bm')?.toString() || null,
    formData.get('name_zh')?.toString() || null,
 ...
 * PRODUCTS
 * ============================================================ */
function uniqueProductSlug(slug: string): string {
  let s = slug; let i = 2;
  while (db.prepare('SELECT id FROM products WHERE slug = ?').get(s)) { s = `${slug}-${i}`; i++; }
  return s;
}
...
async function writePriceHistory(productId: number, adminId: number, before: any, after: any) {
  const fields: Array<['retail_price' | 'wholesale_price' | 'promotion_price', number | null, number | null]> = [
  ...
  const stmt = db.prepare(
    `INSERT INTO price_history (product_id, price_type, old_price, new_price, changed_by)
     VALUES (?, ?, ?, ?, ?)`
  );
...
export async function createProductAction(formData: FormData): Promise<void> {
  ...
  const name_en = formData.get('name_en')?.toString().trim() ?? '';
  if (!name_en) return;
  const slug = uniqueProductSlug(slugify(name_en));
  ...
  const info = db.prepare(
    ...
    formData.get('name_bm')?.toString() || null,
    formData.get('name_zh')?.toString() || null,
    Number(formData.get('brand_id') || 0) || null,
    Number(formData.get('category_id') || 0) || null,
    formData.get('model')?.toString() || null,
    formData.get('capacity')?.toString() || null,
    formData.get('product_type')?.toString() || null,
    formData.get('description_en')?.toString() || null,
    formData.get('description_bm')?.toString() || null,
    formData.get('description_zh')?.toString() || null,
    formData.get('specifications')?.toString() || null,
    formData.get('stock_status')?.toString() || 'in_stock',
    numOrNull(formData.get('retail_price')),
    numOrNull(formData.get('wholesale_price')),
    numOrNull(formData.get('promotion_price')),
    numOrNull(formData.get('price_min')),
    numOrNull(formData.get('price_max')),
    formData.get('currency')?.toString() || 'RM',
    formData.get('price_display_mode')?.toString() || 'SHOW_PRICE',
    formData.get('featured') ? 1 : 0,
    formData.get('status') === 'off' ? 0 : 1,
    formData.get('seo_title_en')?.toString() || null,
    formData.get('seo_description_en')?.toString() || null
  );
...
export async function updateProductAction(id: number, formData: FormData): Promise<void> {
  ...
  const before = db.prepare(
    'SELECT retail_price, wholesale_price, promotion_price FROM products WHERE id = ?'
  ).get(id) as { retail_price: number | null; wholesale_price: number | null; promotion_price: number | null } | undefined;
...
export async function duplicateProductAction(id: number): Promise<void> {
  ...
  if (!row) return;
  const newSlug = uniqueProductSlug(`${row.slug}-copy`);
  db.prepare(
    `INSERT INTO products (slug, sku, name_en, name_bm, name_zh, brand_id, category_id, model, capacity, product_type,
       description_en, description_bm, description_zh, specifications,
       stock_status, retail_price, wholesale_price, promotion_price, price_min, price_max, currency, price_display_mode,
    ...
    row.sku,
    `${row.name_en} (Copy)`,
    row.name_bm, row.name_zh,
    row.brand_id, row.category_id, row.model, row.capacity, row.product_type,
    row.description_en, row.description_bm, row.description_zh, row.specifications,
    row.stock_status, row.retail_price, row.wholesale_price, row.promotion_price,
    row.price_min, row.price_max, row.currency, row.price_display_mode,
    0, row.status
  );
  revalidatePath('/[lang]/products', 'page');
}
...
export async function addProductMediaAction(productId: number, formData: FormData): Promise<void> {
  ...
  if (isPrimary) {
    db.prepare('UPDATE product_media SET is_primary = 0 WHERE product_id = ?').run(productId);
  }
  db.prepare(
  ...
  ).run(productId, type, url, alt, productId, isPrimary);
...
export async function setPrimaryMediaAction(mediaId: number, productId: number): Promise<void> {
  ...
  if (!admin) redirect('/admin/login');
  db.prepare('UPDATE product_media SET is_primary = 0 WHERE product_id = ?').run(productId);
  db.prepare('UPDATE product_media SET is_primary = 1 WHERE id = ?').run(mediaId);
  revalidatePath('/admin/products');
}
...
export async function reorderProductMediaAction(productId: number, orderedIds: number[]): Promise<void> {
  ...
  const tx = db.transaction((ids: number[]) => {
    ids.forEach((id, idx) => stmt.run(idx + 1, id, productId));
  });
  tx(orderedIds);
  revalidatePath('/admin/products');
}
 ...
 * LOCATIONS
 * ============================================================ */
function uniqueLocationSlug(slug: string): string {
  let s = slug; let i = 2;
  while (db.prepare('SELECT id FROM locations WHERE slug = ?').get(s)) { s = `${slug}-${i}`; i++; }
  return s;
}
...
export async function createLocationAction(formData: FormData): Promise<void> {
  ...
  const name_en = formData.get('name_en')?.toString().trim() ?? '';
  if (!name_en) return;
  const slug = uniqueLocationSlug(slugify(name_en));
  db.prepare(
    `INSERT INTO locations (slug, name_en, name_bm, name_zh, type, is_hq, address, city, state, postal_code,
    ...
    formData.get('name_bm')?.toString() || null,
    formData.get('name_zh')?.toString() || null,
    formData.get('type')?.toString() || 'branch',
    formData.get('is_hq') ? 1 : 0,
    formData.get('address')?.toString() || null,
    formData.get('city')?.toString() || null,
    formData.get('state')?.toString() || null,
    formData.get('postal_code')?.toString() || null,
    formData.get('country')?.toString() || 'Malaysia',
    formData.get('telephone')?.toString() || null,
    formData.get('whatsapp')?.toString() || null,
    formData.get('email')?.toString() || null,
    formData.get('opening_hours')?.toString() || null,
    formData.get('google_maps_url')?.toString() || null,
    formData.get('google_maps_place_id')?.toString() || null,
    numOrNull(formData.get('latitude')),
    numOrNull(formData.get('longitude')),
    formData.get('photo_url')?.toString() || null,
    formData.get('description_en')?.toString() || null,
    formData.get('description_bm')?.toString() || null,
    formData.get('description_zh')?.toString() || null,
    Number(formData.get('display_order') || 0),
 ...
 * TECHNICAL PARTNERS
 * ============================================================ */
function uniquePartnerSlug(slug: string): string {
  let s = slug; let i = 2;
  while (db.prepare('SELECT id FROM technical_partners WHERE slug = ?').get(s)) { s = `${slug}-${i}`; i++; }
  return s;
}
...
export async function createPartnerAction(formData: FormData): Promise<void> {
  ...
  if (!name_en) return;
  const slug = uniquePartnerSlug(slugify(name_en));
  db.prepare(
    ...
    slug,
    name_en,
    formData.get('company_name_bm')?.toString() || null,
    formData.get('company_name_zh')?.toString() || null,
    formData.get('contact_person')?.toString() || null,
    formData.get('telephone')?.toString() || null,
    formData.get('whatsapp')?.toString() || null,
    formData.get('email')?.toString() || null,
    formData.get('address')?.toString() || null,
    ...
    formData.get('state')?.toString() || null,
    formData.get('country')?.toString() || 'Malaysia',
    formData.get('service_area')?.toString() || null,
    formData.get('service_types')?.toString() || null,
    formData.get('logo_url')?.toString() || null,
    formData.get('photo_url')?.toString() || null,
    formData.get('description_en')?.toString() || null,
    formData.get('description_bm')?.toString() || null,
    formData.get('description_zh')?.toString() || null,
    formData.get('website')?.toString() || null,
    formData.get('facebook')?.toString() || null,
    formData.get('google_maps_url')?.toString() || null,
    Number(formData.get('display_order') || 0),
    formData.get('featured') ? 1 : 0,
    formData.get('status') === 'off' ? 0 : 1,
    formData.get('show_phone') === 'off' ? 0 : 1,
    formData.get('show_whatsapp') === 'off' ? 0 : 1,
    formData.get('show_email') === 'off' ? 0 : 1,
    formData.get('show_address') === 'off' ? 0 : 1,
    formData.get('show_website') === 'off' ? 0 : 1
  );
...
export async function updatePartnerAction(id: number, formData: FormData): Promise<void> {
  ...
  db.prepare(
    ...
    formData.get('company_name_en')?.toString() || '',
    formData.get('company_name_bm')?.toString() || null,
    formData.get('company_name_zh')?.toString() || null,
    formData.get('contact_person')?.toString() || null,
    formData.get('telephone')?.toString() || null,
...
export async function createFaqAction(formData: FormData): Promise<void> {
  ...
  if (!q_en || !a_en) return;
  db.prepare(
    `INSERT INTO faqs (category, question_en, question_bm, question_zh, answer_en, answer_bm, answer_zh, display_order, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    formData.get('category')?.toString() || null,
    q_en,
    formData.get('question_bm')?.toString() || null,
    formData.get('question_zh')?.toString() || null,
    a_en,
    formData.get('answer_bm')?.toString() || null,
    formData.get('answer_zh')?.toString() || null,
    Number(formData.get('display_order') || 0),
    formData.get('status') === 'off' ? 0 : 1
  );
}
...
export async function updateFaqAction(id: number, formData: FormData): Promise<void> {
  ...
  if (!admin) redirect('/admin/login');
  db.prepare(
    `UPDATE faqs SET category = ?, question_en = ?, question_bm = ?, question_zh = ?,
       answer_en = ?, answer_bm = ?, answer_zh = ?, display_order = ?, status = ? WHERE id = ?`
  ).run(
    formData.get('category')?.toString() || null,
    formData.get('question_en')?.toString() || '',
    formData.get('question_bm')?.toString() || null,
    formData.get('question_zh')?.toString() || null,
    formData.get('answer_en')?.toString() || '',
    formData.get('answer_bm')?.toString() || null,
    formData.get('answer_zh')?.toString() || null,
    Number(formData.get('display_order') || 0),
    formData.get('status') === 'off' ? 0 : 1,
 ...
 * SETTINGS
 * ============================================================ */
export async function saveSettingsAction(formData: FormData): Promise<{ ok: boolean }> {
  const admin = await getCurrentAdmin();
  if (!admin) redirect('/admin/login');
...
export async function upsertHomepageSectionAction(formData: FormData): Promise<void> {
  ...
  db.prepare(
    ...
    formData.get('title_en')?.toString() || null,
    formData.get('title_bm')?.toString() || null,
    formData.get('title_zh')?.toString() || null,
    formData.get('subtitle_en')?.toString() || null,
    formData.get('subtitle_bm')?.toString() || null,
    formData.get('subtitle_zh')?.toString() || null,
    formData.get('body_en')?.toString() || null,
    formData.get('body_bm')?.toString() || null,
    formData.get('body_zh')?.toString() || null,
    formData.get('image_url')?.toString() || null,
    formData.get('video_url')?.toString() || null,
    formData.get('cta_label_en')?.toString() || null,
    formData.get('cta_label_bm')?.toString() || null,
    formData.get('cta_label_zh')?.toString() || null,
    formData.get('cta_url')?.toString() || null,
    Number(formData.get('display_order') || 0)
  );
 ...
 * ADMIN USER CREATION
 * ============================================================ */
export async function createAdminAction(formData: FormData): Promise<{ ok: boolean; error?: string }> {
  const admin = await getCurrentAdmin();
  if (!admin) redirect('/admin/login');
  ...
  db.prepare('INSERT INTO admin_users (email, name, password_hash, role) VALUES (?, ?, ?, ?)').run(email, name, hash, 'admin');
  return { ok: true };
}
