...
export default async function PartsPage({ params, searchParams, }: {
    ...
    // Children of every non-aircon group (slug fallback aware).
    const partsCategories = (
      await Promise.all(partsGroupIds.map((g) => data.listChildCategories(g)))
    ).flat();
    const partsCategoryIds = new Set(partsCategories.map((c) => c.id));
    const category = sp.category
        ? categories.find((c) => c.slug === sp.category || String(c.id) === sp.category)
        : undefined;
    const products = await data.searchProducts({
        q: sp.q,
        categoryId: category && partsCategoryIds.has(category.id) ? category.id : undefined,
    ...
    return (<div className="container-fluid py-8">
      ...
      <form className="card p-4 mb-6 grid sm:grid-cols-4 gap-3">
        ...
        <select name="category" defaultValue={sp.category ?? ''} className="input">
          ...
          {partsCategories.map((c) => (<option key={c.id} value={c.slug}>{c.name_en}</option>))}
        </select>
