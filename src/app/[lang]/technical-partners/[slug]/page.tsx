...
export default async function PartnerDetailPage({ params, }: {
    ...
    return (<div className="container-fluid py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <img src={partner.logo_url} alt={name} className="h-20 w-20 object-contain rounded-md border border-gray-100 bg-white"/>) : (<div className="h-20 w-20 rounded-md bg-brand-100 flex items-center justify-center text-brand-700 font-bold text-3xl">
                {name.charAt(0)}
              </div>)}
            <div className="flex-1">
              <h1 className="heading-2 mb-2">{name}</h1>
              {partner.contact_person && (<p className="text-sm text-gray-600">Contact: {partner.contact_person}</p>)}
              {partner.service_area && (<p className="text-sm text-gray-500 mt-1">{t(lang, 'partners.serviceArea')}: {partner.service_area}</p>)}
              {serviceTypes.length > 0 && (<div className="flex flex-wrap gap-1 mt-3">
                  {serviceTypes.map((s) => (<span key={s} className="badge-blue">{t(lang, `partners.serviceTypes.${s}`) || s}</span>))}
                </div>)}
            </div>
          </div>

          {desc && (<div className="card p-6 mb-6">
              <h3 className="heading-3 mb-3">Description</h3>
              <p className="text-gray-600 leading-relaxed whitespace-pre-line">{desc}</p>
            </div>)}

          {partner.photo_url && (<div className="card overflow-hidden">
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <aside>
          <div className="card p-5 sticky top-32">
            ...
            <ul className="space-y-3 text-sm">
              {partner.show_phone === 1 && partner.telephone && (<li>
              ...
              {partner.show_whatsapp === 1 && partner.whatsapp && (<li>
                  <div className="text-xs text-gray-500 uppercase tracking-wider mb-0.5">WhatsApp</div>
                  <a href={`https://wa.me/${partner.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="font-medium text-brand-700">
                    💬 {partner.whatsapp}
                  </a>
                </li>)}
              {partner.show_email === 1 && partner.email && (<li>
              ...
              {partner.show_address === 1 && partner.address && (<li>
              ...
              {partner.show_website === 1 && partner.website && (<li>
            ...
            <div className="mt-5 space-y-2">
              {partner.telephone && (<a href={`tel:${partner.telephone.replace(/\s/g, '')}`} className="btn-primary w-full">{t(lang, 'common.callNow')}</a>)}
              {partner.whatsapp && (<a href={`https://wa.me/${partner.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="btn-whatsapp w-full">
                  WhatsApp
                </a>)}
              {partner.show_website === 1 && partner.website && (<a href={partner.website} target="_blank" rel="noopener noreferrer" className="btn-secondary w-full">Visit Website</a>)}
              {directions && (<a href={directions} target="_blank" rel="noopener noreferrer" className="btn-secondary w-full">{t(lang, 'locations.directions')}</a>)}
            </div>
          </div>
        </aside>
      </div>
