...
export default async function BrandsListPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang: Locale = (LOCALES as readonly string[]).includes(rawLang) ? (rawLang as Locale) : 'en';
  ...
  return (
    <div className="container-fluid py-8">
      ...
      {brands.length === 0 ? (
        <div className="card p-12 text-center text-gray-500">{t(lang, 'brands.noBrands')}</div>
      ) : (
