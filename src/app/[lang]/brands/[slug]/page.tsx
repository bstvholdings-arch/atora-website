...
export async function generateMetadata({
  ...
  const brand = data.getBrandBySlug(slug);
  if (!brand) return { title: 'Brand — ATORA' };
...
export default async function BrandDetailPage({
  ...
  const products = data.listBrandProducts(brand.id);
    ...
    <div className="container-fluid py-8">
      ...
      <header className="card p-6 sm:p-8 mb-8 flex flex-col sm:flex-row gap-6 items-center">
        {brand.logo ? (
          ...
          <img src={brand.logo} alt={name} className="h-20 w-auto" />
        ) : (
      ...
      {products.length === 0 ? (
        <div className="card p-12 text-center text-gray-500">{t(lang, 'products.noResults')}</div>
      ) : (
