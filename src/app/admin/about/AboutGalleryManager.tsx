...
/**
 * AboutGalleryManager — "照片画廊" section of the About Us admin page.
 * Stores rows in about_gallery (cover = is_primary, order = display_order).
 * Reuses the photo validation helper from the product album.
 */
import { useEffect, useRef, useState } from 'react';
import { useRouter } from 'next/navigation';
import type { AboutPhoto } from '@/lib/db';
import {
    listAboutPhotosAction,
    addAboutPhotosAction,
    deleteAboutPhotoAction,
    setAboutCoverAction,
    reorderAboutPhotosAction,
} from '@/lib/actions';
...
export default function AboutGalleryManager() {
    ...
    async function reload() {
        setPhotos(await listAboutPhotosAction());
    }
    ...
    async function uploadFiles(fileList: FileList | null) {
        ...
        setBusy(true);
        const uploaded: { url: string; file_name: string | null; alt_text: string | null }[] = [];
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                uploaded.push({ url: data.url as string, file_name: file.name, alt_text: null });
            }
            if (uploaded.length > 0) {
                const result = await addAboutPhotosAction(uploaded, false);
                if (!result.ok)
                    setError(result.error ?? 'Failed to save photos.');
                else {
                    setNotice(`${result.added ?? uploaded.length} photo(s) added.`);
                    await reload();
                    router.refresh();
                }
            }
    ...
    async function onDelete(id: number) {
        ...
        setBusy(true);
        try {
            await deleteAboutPhotoAction(id);
            await reload();
            router.refresh();
    ...
    async function persistOrder(next: AboutPhoto[]) {
        setBusy(true);
        try {
            await reorderAboutPhotosAction(next.map((p) => p.id));
            await reload();
            router.refresh();
        }
        finally {
    ...
    return (
        <div className="rounded-md bg-gray-50 p-4 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                    <h3 className="font-semibold text-brand-800">照片画廊 · Photo Gallery</h3>
                    <p className="text-xs text-gray-500 mt-0.5">
                        JPG / PNG / GIF only, max 5MB each. Drag to reorder or use ↑ ↓. The cover photo shows first.
                    </p>
                </div>
                    ...
                    {ordered.map((p, i) => (
                        ...
                        >
                            ...
                            <div className="mt-2 flex flex-wrap gap-1 text-[11px]">
                                ...
                                <button type="button" disabled={busy} onClick={() => onDelete(p.id)} className="text-red-600 hover:text-red-700 disabled:opacity-40">
                                    Delete
                                </button>
                            </div>
                        </div>
